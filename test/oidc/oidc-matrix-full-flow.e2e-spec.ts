  ...
  describe('Matrix Session Cookie Flow', () => {
    it('should complete full Matrix SSO → OIDC → Callback flow with session cookies', async () => {
      ...
      const ssoResponse = await request(MATRIX_BASE_URL)
        ...
        .query({
          redirectUrl: 'element://vector/webapp/?element-desktop-ssoid=test-full-flow'
        })
      ...
      console.log(`Location: ${ssoResponse.headers.location?.substring(0, 100)}...`);
      ...
      console.log(`📋 OIDC session cookie: ${oidcSessionCookie ? 'Present' : 'Missing'}`);
      
      if (oidcSessionCookie) {
        console.log(`📋 OIDC session cookie length: ${oidcSessionCookie.length} characters`);
        console.log(`📋 Original macaroon value length: ${originalMacaroonValue.length} characters`);
        console.log(`📋 Original macaroon preview: ${originalMacaroonValue.substring(0, 50)}...`);
        
        // Try to analyze the macaroon structure
        try {
          console.log(`📋 Macaroon appears to be base64: ${/^[A-Za-z0-9+/]+=*$/.test(originalMacaroonValue)}`);
          
          // Try to decode as base64 and see if it looks like binary data
          const decodedBytes = Buffer.from(originalMacaroonValue, 'base64');
          console.log(`📋 Decoded macaroon bytes length: ${decodedBytes.length}`);
          ...
          const decodedText = decodedBytes.toString('utf8', 0, Math.min(100, decodedBytes.length));
          console.log(`📋 Decoded text preview: ${decodedText.replace(/[^\x20-\x7E]/g, '?')}`);
        } catch (error) {
      ...
      const authCodeResponse = await request(process.env.BACKEND_DOMAIN || 'http://localhost:3000')
      ...
      console.log('📋 Step 4: OIDC authorization with auth code and session cookies');
      
      const oidcAuthResponse = await request(process.env.BACKEND_DOMAIN || 'http://localhost:3000')
        .get('/api/oidc/auth')
        .query({
          client_id: clientId,
          response_type: 'code',
      ...
      console.log('📋 Step 6: Matrix OIDC callback - testing session cookie validation');
      ...
      console.log(`   Original macaroon: ${originalMacaroonValue.substring(0, 50)}...`);
      console.log(`   Sending back:     ${cookieValueToSend.substring(0, 50)}...`);
      console.log(`   Values match:     ${originalMacaroonValue === cookieValueToSend}`);
      ...
      if (originalMacaroonValue !== cookieValueToSend) {
        ...
        console.log('❌ We are corrupting the Matrix session cookie ourselves!');
        
        // Find where they differ
        for (let i = 0; i < Math.min(originalMacaroonValue.length, cookieValueToSend.length); i++) {
          if (originalMacaroonValue[i] !== cookieValueToSend[i]) {
            console.log(`   First difference at position ${i}:`);
            console.log(`   Original: '${originalMacaroonValue[i]}' (${originalMacaroonValue.charCodeAt(i)})`);
            console.log(`   Sending:  '${cookieValueToSend[i]}' (${cookieValueToSend.charCodeAt(i)})`);
            break;
          }
        }
        
        throw new Error('We are corrupting the Matrix session cookie - not ALB!');
      }
      ...
      console.log(`Location: ${matrixCallbackResponse.headers.location || 'No redirect'}`);
      console.log(`Response body preview: ${matrixCallbackResponse.text?.substring(0, 200)}...`);
      
      // If it's HTML and potentially an error, log more detail
      if (matrixCallbackResponse.text?.includes('<!DOCTYPE html>')) {
        console.log('📄 HTML Response detected - checking for error content...');
      ...
      const hasMarcaroonError = responseText.includes('MacaroonDeserializationException') || 
                              ...
                              (matrixCallbackResponse.status === 200 && responseText.includes('<!DOCTYPE html>') && 
      ...
      if (matrixCallbackResponse.status === 400) {
        ...
        if (hasMarcaroonError) {
          console.log('🔥 DETECTED: Macaroon deserialization error in response!');
          console.log('❌ ALB is still corrupting Matrix session cookies');
          throw new Error('Matrix macaroon deserialization failed - ALB cookie corruption not fixed');
        } else {
          console.log('❌ Matrix callback failed for other reasons');
          console.log(`Error response: ${responseText}`);
        }
      } else if (matrixCallbackResponse.status === 302) {
        console.log('✅ Matrix callback succeeded - session cookie validation passed');
        console.log('🎉 ALB cookie corruption appears to be FIXED!');
      } else {
        console.log(`⚠️ Unexpected Matrix response status: ${matrixCallbackResponse.status}`);
      }

      // Fail explicitly if we detect macaroon errors regardless of status code
      if (hasMarcaroonError) {
        throw new Error(`Matrix macaroon deserialization error detected: ${responseText.substring(0, 300)}`);
      }
      ...
      if (matrixCallbackResponse.status === 400 && !hasMarcaroonError) {
        console.log('✅ Matrix returned 400 but no macaroon error - this is acceptable');
      } else {
      ...
      console.log('✅ Full Matrix OIDC session cookie flow completed successfully!');
    });
    ...
    it('should handle invalid session cookies gracefully', async () => {
      ...
      const ssoResponse = await request(MATRIX_BASE_URL)
        .get('/_matrix/client/v3/login/sso/redirect/oidc-openmeet')
        .query({
          redirectUrl: 'element://vector/webapp/?element-desktop-ssoid=test-invalid-cookie'
        })
        .set('x-tenant-id', TESTING_TENANT_ID)
        .expect(302);
      ...
      console.log(`Invalid cookie response status: ${matrixCallbackResponse.status}`);
  ...
  describe('Session Cookie Analysis', () => {
    it('should analyze Matrix session cookie structure', async () => {
      ...
      const ssoResponse = await request(MATRIX_BASE_URL)
        .get('/_matrix/client/v3/login/sso/redirect/oidc-openmeet')
        .query({
          redirectUrl: 'element://vector/webapp/?element-desktop-ssoid=test-cookie-analysis'
        })
        .set('x-tenant-id', TESTING_TENANT_ID)
        .expect(302);
      ...
      setCookieHeaders.forEach((cookie: string, index: number) => {
        if (cookie.includes('oidc_session')) {
          ...
          if (match) {
            ...
            console.log(`   - Appears to be base64: ${/^[A-Za-z0-9+/]+=*$/.test(cookieValue)}`);
