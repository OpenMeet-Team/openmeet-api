...
describe('Simple Session Cookie Test', () => {
  ...
  it('should get session cookie and validate it with API', async () => {
    ...
    const ssoResponse = await request(MATRIX_BASE_URL)
      ...
      .query({
        redirectUrl:
          'element://vector/webapp/?element-desktop-ssoid=simple-test',
      })
    ...
    console.log(
      `📋 Session validation response status: ${sessionTestResponse.status}`,
    );
    console.log(
      `📋 Response headers: ${JSON.stringify(sessionTestResponse.headers, null, 2)}`,
    );
