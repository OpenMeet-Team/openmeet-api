...
describe('AuthGithubService', () => {
  ...
  beforeEach(async () => {
    ...
    service = await module.resolve<AuthGithubService>(AuthGithubService);
  });
  ...
  describe('buildRedirectUrl', () => {
    ...
    it('should redirect to frontend domain when platform is web', () => {
      ...
      expect(result).toMatch(
        /^https:\/\/platform\.openmeet\.net\/auth\/github\/callback\?/,
      );
    ...
    it('should redirect to custom URL scheme when platform is android', () => {
      ...
      expect(result).toMatch(
        /^net\.openmeet\.platform:\/auth\/github\/callback\?/,
      );
      expect(result).toContain('token=test-token');
    });
    ...
    it('should use configurable custom URL scheme', () => {
      ...
      expect(result).toMatch(/^com\.custom\.app:\/auth\/github\/callback\?/);
    });
  ...
  describe('handleCallback', () => {
    ...
    beforeEach(async () => {
      ...
      // Mock getProfileByCode to return test profile
      jest.spyOn(service, 'getProfileByCode').mockResolvedValue({
        id: 'github-user-123',
