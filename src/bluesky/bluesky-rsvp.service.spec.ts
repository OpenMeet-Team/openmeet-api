...
describe('BlueskyRsvpService', () => {
  ...
  describe('createRsvp', () => {
    it('should create an RSVP in Bluesky PDS', async () => {
      ...
      // Mock the event URI
      const mockEventUri = 'at://did:plc:abcdef123456/community.lexicon.calendar.event/event123';
      blueskyIdService.createUri.mockReturnValueOnce(mockEventUri);
      
      // Mock the RSVP URI for the return value
      const mockRsvpUri = 'at://did:plc:xyz789/community.lexicon.calendar.rsvp/event123-rsvp-123456789';
      ...
      blueskyService.resumeSession.mockResolvedValue(mockAgent as unknown as Agent);
      
      // Call the service method
      const result = await service.createRsvp(event, 'going', userDid, tenantId);
      ...
      expect(blueskyService.resumeSession).toHaveBeenCalledWith(tenantId, userDid);
  ...
  describe('deleteRsvp', () => {
    it('should delete an RSVP from Bluesky PDS', async () => {
      ...
      const rsvpUri = 'at://did:plc:xyz789/community.lexicon.calendar.rsvp/event123-rsvp-123456789';
      const userDid = 'did:plc:xyz789';
      const tenantId = 'tenant123';
    ...
    it('should throw an error if RSVP URI DID does not match provided DID', async () => {
      ...
      const rsvpUri = 'at://did:plc:different/community.lexicon.calendar.rsvp/event123-rsvp-123456789';
      const userDid = 'did:plc:xyz789';
      const tenantId = 'tenant123';
  ...
  describe('listRsvps', () => {
    it('should list RSVPs from Bluesky PDS', async () => {
      ...
      blueskyIdService.createUri
        .mockReturnValueOnce('at://did:plc:xyz789/community.lexicon.calendar.rsvp/event1-rsvp')
        .mockReturnValueOnce('at://did:plc:xyz789/community.lexicon.calendar.rsvp/event2-rsvp');
  ...
  describe('findRsvpForEvent', () => {
    it('should find an RSVP for a specific event', async () => {
      ...
      const eventUri = 'at://did:plc:abc/community.lexicon.calendar.event/event1';
      const userDid = 'did:plc:xyz789';
      const tenantId = 'tenant123';
      ...
      // Call the service method
      const result = await service.findRsvpForEvent(eventUri, userDid, tenantId);
    ...
    it('should return exists=false if no RSVP found for the event', async () => {
      // Mock event URI, user DID, and tenant ID
      const eventUri = 'at://did:plc:abc/community.lexicon.calendar.event/nonexistent';
      const userDid = 'did:plc:xyz789';
      const tenantId = 'tenant123';
    ...
    it('should handle errors and return exists=false', async () => {
      ...
      jest.spyOn(service, 'listRsvps').mockRejectedValue(new Error('Failed to list RSVPs'));
