...
import { GroupService } from '../../groups/groups.service';
  ...
  export class PermissionsGuard implements CanActivate {
    ...
    private async getUserPermissions(userId: number, groupId?: number): Promise<Set<string>> {
     ...
     if (groupId) {
      ...
      private async getGroupPermissions(userId: number, groupId: number): Promise<Set<string>> {
        const groupPermissions = new Set<string>();
      
        // Fetch the user's group membership
        const groupMember = await this.groupService.getGroupMembers(userId, groupId);
        ...
        const userGroupPermissions = await this.groupService.getGroupMemberPermissions(userId, groupId);
