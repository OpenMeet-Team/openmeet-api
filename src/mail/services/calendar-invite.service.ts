...
export class CalendarInviteService {
  ...
  async sendCalendarInvite(
    ...
    if (!attendee.email) {
      throw new Error('Attendee email is required to send calendar invite');
    }

    // Generate ICS content using ICalendarService
    ...
    await this.mailerService.sendCalendarInviteMail({
      ...
      context: {
        ...
        eventDate: this.formatDateHumanReadable(event.startDate),
        eventTime: this.formatTimeHumanReadable(event.startDate),
        eventUrl,
        attendeeName:
   ...
   */
  private formatDateHumanReadable(date: Date): string {
   ...
   */
  private formatTimeHumanReadable(date: Date): string {
