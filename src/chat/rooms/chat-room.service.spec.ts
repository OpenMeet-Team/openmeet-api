...
import { EventAttendeePermission, EventAttendeeRole, GroupRole, GroupPermission } from '../../core/constants/constant';
...
describe('ChatRoomService', () => {
  ...
  const mockGroupMember = {
    ...
    groupRole: { 
      ...
      permissions: [
        { name: GroupPermission.ManageGroup },
      ],
    },
    group: mockGroup,
    user: mockUserWithMatrix,
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            ...
            getUserBySlug: jest.fn().mockImplementation((slug) => {
              if (slug === 'test-user') return Promise.resolve(mockUserWithMatrix);
              return Promise.resolve(null);
            }),
          },
        },
        ...
        },
        {
          ...
          useValue: {
            findGroupMemberByUserId: jest.fn().mockImplementation((groupId, userId) => {
              if (groupId === mockGroup.id && userId === mockUserWithMatrix.id) {
        ...
        },
        {
          ...
          useValue: {
            findEventAttendeeByUserId: jest.fn().mockImplementation((eventId, userId) => {
              if (eventId === mockEvent.id && userId === mockUserWithMatrix.id) {
            ...
            getTenantConnection: jest.fn().mockImplementation(() => {
              return Promise.resolve({
                getRepository: jest.fn().mockImplementation((entity) => {
                  ...
                  if (entity === 'EventEntity' || entity.name === 'EventEntity') return mockEventRepository;
                  if (entity === 'GroupEntity' || entity.name === 'GroupEntity') return mockGroupRepository;
    ...
    eventAttendeeService = module.get<EventAttendeeService>(EventAttendeeService);
  ...
  describe('ensureUserHasMatrixCredentials', () => {
    ...
    it('should provision Matrix credentials if user does not have them', async () => {
      ...
      expect(matrixService.createUser).toHaveBeenCalledWith(expect.objectContaining({
      ...
      expect(userService.update).toHaveBeenCalledWith(2, expect.objectContaining({
  ...
  describe('getOrCreateEventChatRoom', () => {
    ...
    it('should create a new chat room if none exists', async () => {
      ...
      (matrixService.createRoom as jest.Mock).mockResolvedValueOnce({
        roomId: '!room123:matrix.openmeet.net',
        name: 'Test Room',
      });
      
      // Mock the method that creates the chat room - we need to cast to any to avoid type issues
      jest.spyOn(service, 'createEventChatRoom').mockResolvedValue(mockChatRoom as any);
  ...
  describe('getOrCreateGroupChatRoom', () => {
    it('should return existing group chat room if one exists', async () => {
      // Setup mock repository call to return existing room
      mockChatRoomRepository.findOne = jest.fn().mockResolvedValue(mockGroupChatRoom);
    ...
    it('should create a new group chat room if none exists', async () => {
      ...
      jest.spyOn(service, 'createGroupChatRoom').mockResolvedValue(mockGroupChatRoom as any);
    ...
    it('should throw an error if group is not found', async () => {
      ...
      await expect(service.getOrCreateGroupChatRoom(999)).rejects.toThrow('Group with id 999 not found');
    });
    ...
    it('should throw an error if group has no creator', async () => {
      ...
      // Should throw an error
      await expect(service.getOrCreateGroupChatRoom(mockGroup.id)).rejects.toThrow(
        `Group with id ${mockGroup.id} has no creator`
      );
    });
  });
  ...
  describe('getOrCreateDirectChatRoom', () => {
    it('should return existing direct chat room if one exists', async () => {
      ...
      mockChatRoomRepository.createQueryBuilder = jest.fn().mockReturnValue(mockQueryBuilder);
      
      const result = await service.getOrCreateDirectChatRoom(1, 2);
      
      // Should have called createQueryBuilder to find rooms
      expect(mockChatRoomRepository.createQueryBuilder).toHaveBeenCalledWith('chatRoom');
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('chatRoom.type = :type', { type: ChatRoomType.DIRECT });
    ...
    it('should create a new direct chat room if none exists', async () => {
      ...
      mockChatRoomRepository.createQueryBuilder = jest.fn().mockReturnValue(mockQueryBuilder);
      mockChatRoomRepository.create = jest.fn().mockReturnValue(mockDirectChatRoom);
      mockChatRoomRepository.save = jest.fn().mockResolvedValue(mockDirectChatRoom);
      ...
      jest.spyOn(service as any, 'ensureUserHasMatrixCredentials').mockResolvedValue(mockUserWithMatrix);
      ...
      expect((service as any).ensureUserHasMatrixCredentials).toHaveBeenCalledWith(1);
      
      // Should have called Matrix service to create room
      expect(matrixService.createRoom).toHaveBeenCalledWith(expect.objectContaining({
        isDirect: true,
        isPublic: false,
      }));
      
      // Should have created and saved a new ChatRoomEntity with user1Id and user2Id
      expect(mockChatRoomRepository.create).toHaveBeenCalledWith(expect.objectContaining({
        type: ChatRoomType.DIRECT,
        visibility: ChatRoomVisibility.PRIVATE,
        user1Id: 1,
        user2Id: 2,
  ...
  describe('getEventChatRoomBySlug', () => {
    it('should find a chat room by event slug', async () => {
      ...
      expect(eventQueryService.findEventBySlug).toHaveBeenCalledWith('test-event');
    ...
    it('should return null if event is not found', async () => {
      ...
      (eventQueryService.findEventBySlug as jest.Mock).mockResolvedValueOnce(null);
      
      const result = await service.getEventChatRoomBySlug('non-existent-event');
      
      // Should have called EventQueryService to find the event by slug
      expect(eventQueryService.findEventBySlug).toHaveBeenCalledWith('non-existent-event');
    ...
    it('should handle errors gracefully', async () => {
      ...
      (eventQueryService.findEventBySlug as jest.Mock).mockRejectedValueOnce(new Error('Test error'));
      
      const result = await service.getEventChatRoomBySlug('error-event');
      
      // Should have called EventQueryService to find the event by slug
      expect(eventQueryService.findEventBySlug).toHaveBeenCalledWith('error-event');
  ...
  describe('getGroupChatRoomBySlug', () => {
    ...
    it('should return null if group is not found', async () => {
      ...
      // Should have called GroupService to find the group by slug
      expect(groupService.findGroupBySlug).toHaveBeenCalledWith('non-existent-group');
    ...
    it('should handle errors gracefully', async () => {
      ...
      (groupService.findGroupBySlug as jest.Mock).mockRejectedValueOnce(new Error('Test error'));
  ...
  describe('Slug-based methods', () => {
    it('should add user to event chat room by slugs', async () => {
      ...
      jest.spyOn(service, 'addUserToEventChatRoom').mockResolvedValue(undefined);
      
      // Mock the necessary service methods to avoid real calls
      (userService.getUserBySlug as jest.Mock).mockResolvedValue(mockUserWithMatrix);
      (eventQueryService.findEventBySlug as jest.Mock).mockResolvedValue(mockEvent);
      (eventAttendeeService.findEventAttendeeByUserId as jest.Mock).mockResolvedValue(mockEventAttendee);
    ...
    it('should add user to group chat room by slugs', async () => {
      // Set up spies directly on the current service instance
      jest.spyOn(service, 'addUserToGroupChatRoom').mockResolvedValue(undefined);
      
      // Mock the necessary service methods to avoid real calls
      (userService.getUserBySlug as jest.Mock).mockResolvedValue(mockUserWithMatrix);
      (groupService.findGroupBySlug as jest.Mock).mockResolvedValue(mockGroup);
      (groupMemberService.findGroupMemberByUserId as jest.Mock).mockResolvedValue(mockGroupMember);
