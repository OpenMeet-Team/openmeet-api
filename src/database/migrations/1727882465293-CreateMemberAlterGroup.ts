...
export class CreateMemberAlterGroup1727882465293 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "${schema}"."groupMember" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "groupId" integer, CONSTRAINT "PK_dbf23f6a7b4374ae57b50d262f9" PRIMARY KEY ("id"))`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Group" ADD "slug" character varying(255) NOT NULL DEFAULT 'default-slug'`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Group" ADD "approved" boolean NOT NULL DEFAULT false`);
        await queryRunner.query(`CREATE TYPE "${schema}"."Group_status_enum" AS ENUM('PUBLIC', 'PRIVATE')`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Group" ADD "status" "${schema}"."Group_status_enum"`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Event" ADD "groupId" integer`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "${schema}"."Event" DROP COLUMN "groupId"`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Group" DROP COLUMN "status"`);
        await queryRunner.query(`DROP TYPE "${schema}"."Group_status_enum"`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Group" DROP COLUMN "approved"`);
        await queryRunner.query(`ALTER TABLE "${schema}"."Group" DROP COLUMN "slug"`);
