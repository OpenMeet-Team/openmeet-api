...
export class MatrixCoreService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    try {
      ...
      if (!this.adminAccessToken) {
        this.logger.warn('No admin token available from token manager, waiting for regeneration');
  ...
  async acquireClient(): Promise<MatrixClientWithContext> {
    ...
    let tokenValid = await this.ensureValidAdminToken();
    ...
    try {
      ...
      if (poolClient.client.getAccessToken() !== this.adminAccessToken) {
        ...
        try {
          ...
          throw new Error(`Failed to reinitialize client pool: ${error.message}`);
        }
      }
      ...
      if (
        ...
        this.tokenManager.reportTokenInvalid();
        this.logger.warn(`Token error detected: ${error.message}, reported to token manager`);
      }
    ...
    try {
      ...
      if (client?.client?.stopClient) {
        try {
          ...
          this.logger.debug('Manually stopped Matrix client after failed release');
        } catch (stopError) {
          this.logger.warn(`Failed to stop Matrix client: ${stopError.message}`);
        }
      }
    }
  }
    ...
    try {
      ...
      if (tokenState === 'valid') {
        ...
        if (latestToken && latestToken !== this.adminAccessToken) {
          this.logger.debug('Updating admin token reference from token manager');
