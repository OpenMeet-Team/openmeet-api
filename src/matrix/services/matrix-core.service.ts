...
export class MatrixCoreService implements OnModuleInit, OnModuleDestroy {
  ...
  async acquireClient(): Promise<MatrixClientWithContext> {
    ...
    // If token is not valid, wait for token regeneration and retry
    if (!tokenValid) {
      this.logger.debug(
        'Token not valid, triggering regeneration and waiting for completion',
      );
      ...
      if (regenerated) {
        this.adminAccessToken = this.tokenManager.getAdminToken();
        this.logger.log('Successfully obtained fresh admin token');
      } else {
