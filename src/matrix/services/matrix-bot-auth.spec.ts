...
describe('MatrixBotService - Authentication Integration Tests', () => {
  ...
  beforeEach(async () => {
          ...
          useValue: {
            getSdk: jest.fn().mockReturnValue({
              createClient: jest.fn().mockReturnValue({
                ...
                createRoom: jest.fn().mockResolvedValue({ room_id: '!test:matrix.openmeet.net' }),
    ...
    matrixBotUserService = module.get<MatrixBotUserService>(MatrixBotUserService);
  });
  ...
  describe('AppService Authentication', () => {
    it('should authenticate bot with Application Service token', async () => {
      ...
      expect(matrixBotUserService.getOrCreateBotUser).toHaveBeenCalledWith(testTenantId);
    });
    ...
    it('should invite user to room using AppService authentication', async () => {
      ...
      await expect(service.inviteUser(roomId, userId, testTenantId)).resolves.not.toThrow();
    });
    ...
    it('should kick user from room using AppService authentication', async () => {
      ...
      await expect(service.removeUser(roomId, userId, testTenantId)).resolves.not.toThrow();
    });
  ...
  describe('OIDC Fallback Authentication', () => {
    beforeEach(() => {
      // Mock no AppService token to force OIDC fallback
      configService.get = jest.fn().mockImplementation((key: string, defaultValue?: any) => {
        const config = {
          'ADMIN_EMAIL': 'admin@openmeet.net',
          'ADMIN_PASSWORD': 'test-password',
          'matrix.bot.username': 'openmeet-admin-bot',
    ...
    it('should fallback to OIDC authentication when AppService token not available', async () => {
      ...
      const oidcService = module.get<MatrixBotService>(MatrixBotService);
      
      await oidcService.authenticateBot(testTenantId);
      
      expect(oidcService.isBotAuthenticated()).toBe(true);
      expect(matrixUserService.getClientForUser).toHaveBeenCalledWith(
        `openmeet-bot-${testTenantId}`,
        undefined,
        testTenantId
      );
    });
  ...
  describe('Error Handling', () => {
    it('should handle authentication failures gracefully', async () => {
      ...
      matrixBotUserService.getOrCreateBotUser = jest.fn().mockRejectedValue(new Error('Authentication failed'));
      
      await expect(service.authenticateBot(testTenantId)).rejects.toThrow('Authentication failed');
    ...
    it('should handle room creation failures gracefully', async () => {
      ...
      mockClient.createRoom = jest.fn().mockRejectedValue(new Error('Room creation failed'));
      ...
      await expect(service.createRoom(roomOptions, testTenantId)).rejects.toThrow();
    });
  });
});