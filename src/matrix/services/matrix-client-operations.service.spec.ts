...
describe('MatrixClientOperationsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            generateMatrixUsername: jest
              .fn()
              .mockImplementation((user, tenantId) =>
    ...
    service = module.get<MatrixClientOperationsService>(
      MatrixClientOperationsService,
    );
  ...
  describe('withAdminClient', () => {
    ...
    it('should retry operation after token regeneration on 401 error', async () => {
      ...
      (tokenManager.reportTokenInvalid as jest.Mock).mockResolvedValueOnce(
        true,
      );
    ...
    it('should fail if token regeneration fails', async () => {
      ...
      await expect(service.withAdminClient(mockOperation)).rejects.toBe(
        originalError,
      );
      expect(tokenManager.reportTokenInvalid).toHaveBeenCalled();
    });
    ...
    it('should stop client even if operation throws', async () => {
      ...
      await expect(service.withAdminClient(mockOperation)).rejects.toThrow(
        'Test error',
      );
      expect(mockMatrixClient.stopClient).toHaveBeenCalled();
    });
  ...
  describe('withMatrixClient', () => {
    it('should execute operation with user client successfully', async () => {
      ...
      (tokenManager.getValidUserToken as jest.Mock).mockResolvedValueOnce(
        'user-token',
      );

      const result = await service.withMatrixClient(
        'test-user',
        mockOperation,
        'tenant1',
      );
    ...
    it('should throw error if user token cannot be obtained', async () => {
      ...
      await expect(
        ...
        'Could not obtain valid token for user @test-user_tenant1:example.org',
      );
    ...
    it('should stop client even if operation fails', async () => {
      const mockOperation = jest
        .fn()
        .mockRejectedValue(new Error('User operation failed'));
  ...
  describe('utility methods', () => {
    ...
    it('should execute admin event operation', async () => {
      ...
      const result = await service.withAdminEventOperation(
        'test-event',
        mockOperation,
      );
