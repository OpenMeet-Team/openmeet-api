...
export class MatrixAppServiceController {
  ...
  private async handleEventRoomQuery(localpart: string): Promise<any> {
    try {
      ...
      this.logger.log(`Event exists: ${eventSlug} (ID: ${event.id}), creating room`);
      ...
      try {
        ...
        this.logger.log(`Matrix room created successfully: ${roomResult.roomId} with alias ${roomAlias}`);
        
        // Configure the newly created room and invite attendees
        try {
          await this.configureEventRoom(roomResult.roomId, eventSlug, tenantId, roomAlias);
        } catch (configError) {
          this.logger.warn(`Failed to configure new room ${roomAlias}: ${configError.message}`);
        }
        ...
        if (roomError.message && 
          ...
          this.logger.log(`Matrix room already exists for alias ${roomAlias} - ensuring attendees are invited`);
          ...
          try {
            ...
            this.logger.warn(`Failed to configure existing room ${roomAlias}: ${configError.message}`);
          }
  ...
  private async getGroupBySlugWithTenant(
    ...
    this.logger.debug(`Looking for group with slug: ${groupSlug} in tenant: ${tenantId}`);
    const dataSource =
    ...
    this.logger.debug(`Group query result: ${group ? `Found group ID ${group.id}` : 'No group found'}`);
    return group;
  }
  ...
  private async handleGroupRoomQuery(localpart: string): Promise<any> {
    try {
      ...
      this.logger.log(`Calling getGroupBySlugWithTenant for slug: ${groupSlug}, tenant: ${tenantId}`);
      let group;
      try {
        group = await this.getGroupBySlugWithTenant(groupSlug, tenantId);
        this.logger.log(`Group query result: ${group ? `Found group ID ${group.id}` : 'No group found'}`);
      } catch (error) {
        this.logger.error(`Error in getGroupBySlugWithTenant: ${error.message}`);
        throw error;
      }
      if (!group) {
      ...
      this.logger.log(`Group exists: ${groupSlug} (ID: ${group.id}), creating room`);
      ...
      try {
        ...
        try {
          await this.configureGroupRoom(roomResult.roomId, groupSlug, tenantId, roomAlias);
        } catch (configError) {
          this.logger.warn(`Failed to configure new group room ${roomAlias}: ${configError.message}`);
        }
        ...
        if (roomError.message && 
            (roomError.message.includes('Room alias already taken') ||
             ...
             roomError.message.includes('MatrixError: [400]') ||
             roomError.message.includes('alias already taken'))) {
          this.logger.log(`Matrix room already exists for alias ${roomAlias} - returning success`);
          return {};
        }
        this.logger.error(`Failed to create Matrix room: ${roomError.message}`);
        return { error: 'Room not found' };
   ...
   */
  private async handleRoomCreateEvent(event: any): Promise<void> {
    try {
      const roomId = event.room_id;
      this.logger.log(`Room created: ${roomId} - waiting for alias to configure`);
      // Room setup will happen in handleRoomAliasEvent when we get the alias
    } catch (error) {
      this.logger.error(`Failed to handle room create event: ${error.message}`, error.stack);
    }
  }
  ...
  private async handleRoomAliasEvent(event: any): Promise<void> {
    try {
      ...
      const aliasWithoutHash = alias.startsWith('#') ? alias.substring(1) : alias;
      const [localpart] = aliasWithoutHash.split(':');
      ...
      if (entityType === 'event') {
        ...
        this.logger.warn(`Unknown entity type: ${entityType} for alias ${alias}`);
      }

    } catch (error) {
      this.logger.error(`Failed to handle room alias event: ${error.message}`, error.stack);
    }
  }
   ...
   */
  private async configureEventRoom(roomId: string | null, eventSlug: string, tenantId: string, alias: string): Promise<void> {
    try {
      this.logger.log(`Configuring event room ${roomId || alias} for event ${eventSlug} in tenant ${tenantId}`);
      ...
      this.logger.log(`Found ${confirmedAttendees.length} confirmed attendees for event ${eventSlug}`);

      // Get Matrix server name from tenant config
      const tenantConfig = getTenantConfig(tenantId);
      const serverName = tenantConfig?.matrixConfig?.serverName;
      if (!serverName) {
        this.logger.error(`Matrix server name not configured for tenant: ${tenantId}`);
        return;
      }
      ...
      for (const attendee of confirmedAttendees) {
        try {
          ...
          if (!matrixHandleRegistration) {
            this.logger.debug(`User ${attendee.user.slug} has no Matrix handle, skipping`);
            continue;
          }
          ...
          this.logger.debug(`Inviting attendee ${userMatrixId} to room ${alias}`);

          // Invite user to the Matrix room using room alias
          await this.matrixRoomService.inviteUser(alias, userMatrixId);
          successCount++;

          this.logger.debug(`Successfully invited attendee ${userMatrixId} to room ${alias}`);
        } catch (error) {
          errorCount++;
          this.logger.error(`Failed to invite attendee ${attendee.user.slug} to Matrix room: ${error.message}`);
        }
      }

      this.logger.log(`Event room invitation completed: ${successCount} users invited, ${errorCount} errors`);

    } catch (error) {
      this.logger.error(`Failed to configure event room: ${error.message}`, error.stack);
    }
  }
   ...
   */
  private async configureGroupRoom(roomId: string, groupSlug: string, tenantId: string, alias: string): Promise<void> {
    try {
      this.logger.log(`Configuring group room ${roomId} for group ${groupSlug} in tenant ${tenantId}`);
      ...
      this.logger.log(`TODO: Get group members for group ${groupSlug} and invite to room ${alias}`);

    } catch (error) {
      this.logger.error(`Failed to configure group room: ${error.message}`, error.stack);
    }
  }
}
