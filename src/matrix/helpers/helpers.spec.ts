...
describe('Matrix Gateway Helpers', () => {
  describe('BroadcastManager', () => {
    ...
    it('should detect duplicate broadcasts', () => {
      ...
      expect(broadcastManager.shouldSkipDuplicateBroadcast(roomId, event)).toBe(false);

      // Second identical broadcast should be skipped
      expect(broadcastManager.shouldSkipDuplicateBroadcast(roomId, event)).toBe(true);

      // Different event should not be skipped
      const differentEvent = { event_id: 'event2', type: 'test-event' };
      expect(broadcastManager.shouldSkipDuplicateBroadcast(roomId, differentEvent)).toBe(false);

      // Different room should not be skipped
      const differentRoom = 'test-room-2';
      expect(broadcastManager.shouldSkipDuplicateBroadcast(differentRoom, event)).toBe(false);
    });
    ...
    it('should clean up old broadcasts', () => {
      ...
      expect(broadcastManager.shouldSkipDuplicateBroadcast(roomId, event1)).toBe(false);
      expect(broadcastManager.shouldSkipDuplicateBroadcast(roomId, event2)).toBe(false);
  ...
  describe('RoomMembershipManager', () => {
    ...
    it('should track room memberships', () => {
      ...
      expect(roomMembershipManager.getUserRooms(matrixUserId).has(roomId1)).toBe(false);
      expect(roomMembershipManager.getUserRooms(matrixUserId).has(roomId2)).toBe(true);
  ...
  describe('TypingManager', () => {
    ...
    it('should debounce typing notifications', () => {
      ...
      expect(typingManager.shouldSendTypingNotification(userId, roomId, true)).toBe(true);
      
      // Same state within window should be debounced
      expect(typingManager.shouldSendTypingNotification(userId, roomId, true)).toBe(false);
      
      // Different state should be sent
      expect(typingManager.shouldSendTypingNotification(userId, roomId, false)).toBe(true);
      
      // Different user should be sent
      expect(typingManager.shouldSendTypingNotification('user2', roomId, true)).toBe(true);
      
      // Different room should be sent
      expect(typingManager.shouldSendTypingNotification(userId, 'room2', true)).toBe(true);
    });
  ...
  describe('MatrixGatewayHelper', () => {
    ...
    it('should get tenant ID from various sources', () => {
      ...
      expect(MatrixGatewayHelper.getTenantId(clientWithTenantData as any)).toBe('tenant1');
      
      // From request data
      const clientWithoutData = { data: {} };
      const requestData = { tenantId: 'tenant2' };
      expect(MatrixGatewayHelper.getTenantId(clientWithoutData as any, requestData)).toBe('tenant2');
      
      // Priority: client data > request data
      expect(MatrixGatewayHelper.getTenantId(clientWithTenantData as any, requestData)).toBe('tenant1');
    });
  });
});