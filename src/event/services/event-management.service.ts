    ...
    for (const event of events) {
      try {
        ...
        if (this.discussionService) {
          ...
          this.logger.log(`Cleaned up chat rooms for event ${event.slug} (ID: ${event.id})`);
        }
        
        // Now that chat rooms are deleted, we can delete the event itself
        await this.eventRepository.delete(event.id);
        this.logger.log(`Deleted event ${event.slug} (ID: ${event.id}) from group ${groupId}`);
      } catch (error) {
        this.logger.error(`Error deleting event ${event.id} from group ${groupId}: ${error.message}`);
